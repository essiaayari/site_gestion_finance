...
@Component({
  selector: 'app-formulaire-facture-client',
  templateUrl: './formulaire-facture-client.component.html',
  styleUrl: './formulaire-facture-client.component.css'
})
export class FormulaireFactureClientComponent  {
  ...
  constructor(private facture_clientservice:FactureClientService,private companyservice:CompanyDataService,private afs:AngularFirestore,private renderer: Renderer2, private el: ElementRef,private modalService: NgbModal,private route:ActivatedRoute,private clientservice:ClientService ){}
...
this.getcompanydata();
    ...
    addfacture_client(){
      ...
      this.facture_clientservice.addfacture_client(this.facture_clientobj);
      this.resetform();
    ...
    chequeValues: { montant_cheque: number, date_versement: string }[] = [];

    addCheque() {
      this.chequeValues.push({ montant_cheque: 0, date_versement: '' });
    }
    updateChequeValues(index: number, property: string, value: any) {
      if (property === 'montant_cheque' || property === 'date_versement') {
        (this.chequeValues[index] as any)[property] = value;
      }
    }
    
    deleteCheque(index: number) {
      this.chequeValues.splice(index, 1);
    }
...
// pour imprimer la facture
    ...
    printInvoice(): void {
      const printContents = this.invoiceContainer.nativeElement.innerHTML;
  ...
  // pour le nombres des chéques
    getChequeNumbers(): number[] {
      // Assuming nombre_cheque is a positive integer
      return Array.from({ length: this.nombre_cheque }, (_, index) => index + 1);
    }
